...
class Kohana_Database_MySQL extends Database {
	...
	public function connect()
	{
		...
		extract(Kohana::$config->load('database.default.connection') + array(
			'database'   => '',
			'hostname'   => '',
			'username'   => '',
			'password'   => '',
	...
	public function exec($type, $parametros, $params = NULL){
		// Make sure the database is connected
		$this->_connection or $this->connect();
	
		$sFileName = strtolower(str_ireplace('Model_','',get_class($this))) . '.xml';
		$sqlXML = simplexml_load_file(APPPATH.'classes/Model/xml/'.$sFileName);
		$sql = (string)$sqlXML->{$type}['value'];
		...
		if ($params == "return"){
			...
			while ($res = mysqli_fetch_assoc($result)){
				$valoresConvertidos = array();
				foreach ($res as $key => $value){
					$key = utf8_encode($key);
					$value = utf8_encode($value);
					$valoresConvertidos[$key] = $value;
				}
				$resultado[] = $valoresConvertidos;
			}
			return $resultado;
		}else{
